const generateUniqueId=(name: string, email: string, mobileNumber: string): Promise<string> =>{
    ...
    return crypto.subtle.digest("SHA-256", data).then((hashBuffer: ArrayBuffer) => {

      const hashArray = new Uint8Array(hashBuffer);
      const hashHex = Array.from(hashArray).map(byte =>
